...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        echo json_encode(['message' => 'Error decoding JSON: ' . json_last_error_msg()]);
        exit;
    }

    if (isset($data['selected_questions']) && !empty($data['selected_questions'])) {
        ...
        foreach ($selected_questions as $question) {
            $question_type = strtolower(str_replace(' ', '', $question['type']));
            ...
            $questiontext_element->addChild('text', '<![CDATA[' . htmlspecialchars($question['question']) . ']]>');
            ...
            if ($question_type == 'multichoice') {
                ...
                foreach ($question['options'] as $option) {
                    $answer = $question_element->addChild('answer', '<![CDATA[' . htmlspecialchars($option) . ']]>');
        ...
        $desktop_path = $_SERVER['HOME'] . '/Desktop';
        if (!file_exists($desktop_path)) {
            mkdir($desktop_path, 0777, true);
        }
        $xml_filepath = $desktop_path . '/generated_questions.xml';

        // Save the XML to the desktop
        $xml->asXML($xml_filepath);

        // Redirect to the tutorial page with the XML file path
        $redirect_url = new moodle_url('/local/questiongenerator/tutorial.php', array('xml' => $xml_filepath));
        echo json_encode(['redirect' => $redirect_url->out()]);
        exit();
    } else {
        echo json_encode(['message' => 'No questions selected, please go back and select questions.']);
        exit();
    }
}
?>
